...
bool detectCycleDirectedDfs(vector<ll> adj[], vector<bool>& vis, vector<bool>& dfsvis, ll node)
{
    vis[node] = 1;
    dfsvis[node] = 1;
    for(auto &it : adj[node])
    {
        if(!vis[it])
        {
            if(detectCycleDirectedDfs(adj, vis, dfsvis, it))
                return true;
        }
        else if(dfsvis[it])
            return true;
...
//    clock_t tStart = clock();
    ...
    for(ll i = 1;i <= n;i++)
    {
        if(!vis[i])
        {
            if(detectCycleDirectedDfs(adj, vis, dfsvis, i))
            {
                cout << true;
                return 0;
            }
